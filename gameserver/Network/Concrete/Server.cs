...
namespace gameserver.Concrete
{
    class Server : IServer
    {
        ...
        //-----------------added----------
        //и еще я в этом классе закомментил в трех местах отправку данных через клиента, т к у нас теперь все централизовано
        //еще закомментил раздельное управление клиентами 
        public List<Command> GetCommands()
        {
            List<Command> res = new List<Command>();
            Dictionary<int, int> countReceivedCommands = new Dictionary<int, int>();
            foreach (var client in clients)
            {
                List<byte> data= new List<byte>(client.ReadData());
                while (data.Count > 0)
                {
                    Command newCommand = Command.CreateConcrete(ref data);
                    newCommand.clientID = client.Identifier;
                    res.Add(newCommand);

                    if (countReceivedCommands.ContainsKey(client.Identifier))
                        countReceivedCommands[client.Identifier]++;
                    else
                        countReceivedCommands.Add(client.Identifier, 1);
                }
            }

            List<Command> counts = new List<Command>();
            foreach (var a in countReceivedCommands)
            {
                counts.Add(new ComReceivedCommandsCount(a.Key, a.Value));
            }
            SendCommands(counts);
            return res;
        }
        ...
        {
            clients.Sort((a,b)=> a.Identifier.CompareTo(b.Identifier)); 
            commands.Sort((a,b)=> a.clientID.CompareTo(b.clientID)); //сортируем, чтобы все сделать в один проход
            int curClient=0;
            for(int i =0; i < commands.Count; i++)
            {
                try
                {
                    while (commands[i].clientID != clients[curClient].Identifier) curClient++;
                }
                catch (ArgumentOutOfRangeException) //todo тут тоже разные фигни могут случиться. Может быть он отвалился, пока мы ему команду создавали. Надо получше расписать
                {
                    
                    throw new Exception("клиента с id=" + commands[i].id.ToString() + " не существует. Проверьте, правильно ли вы задаете command.clientID");
                }

                clients[curClient].WriteData(commands[i].GetByteData().ToArray());
            }
        }
        //--------------------------
        ...
        private void listenForClients()
        {
            try
            {
                while (true)
                {
                    ...
                    {
                        //todo это должно обрабатываться на более высоком уровне? если клиента не присоединили, ему без разницы, в чем проблема. А спросить свой ник и айди он может через команды
                ...
                } //todo тут точно не надо никакй слип поставить? вечные циклы едят много процессорного времени
            }
        ...
        private void acceptClient(TcpClient connection)
        {
            ...
            clients.Add(client);
            //todo я убрал отсюда отправку стартовых данных, т к этот класс такими вещами не должен заниматься.
            //т к это самый внутренний уровень
        ...
        private void handleClient(IClient client)
        {
            ...
            catch (IOException) {return;} //todo так очень опасно делать
            closeClient(client);
        }
