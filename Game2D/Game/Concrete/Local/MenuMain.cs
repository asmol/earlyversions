...
namespace Game2D.Game.Concrete
{
    class MenuMain
    {
        ...
        public void Process(ref Frame frame, IGetKeyboardState keyboard, DStateMain state)
        {
            _connectionInfo = state.connectionInfo;
            _battleConfig = state.battleConfig;
            _state = state;
            ...
            if (_showIpMenu) _redactorIp.Process(ref frame, keyboard, state.connectionInfo);
        ...
        {
            _state.wish = DStateMain.EWish.joinServer;
        }
